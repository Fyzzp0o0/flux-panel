...
func (p *program) Init(env svc.Environment) error {
	// 面板模式：无任何配置来源时，自动使用 gost.json（不存在则创建空配置），
	// 等待面板通过 WebSocket 下发服务配置（写 gost.json + SIGHUP 热重载）
	if strings.TrimSpace(cfgFile) == "" && len(services) == 0 && len(nodes) == 0 && apiAddr == "" {
		if _, err := os.Stat("gost.json"); os.IsNotExist(err) {
			_ = os.WriteFile("gost.json", []byte("{}"), 0644)
		}
		cfgFile = "gost.json"
	}
...
func (p *program) Start() error {
	...
	// 面板扩展：启动 WebSocket 实时上报 + HTTP 流量上报
	panel.StartPanelServices()
