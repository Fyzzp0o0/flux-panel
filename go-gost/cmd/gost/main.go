...
func main() {
	...
	// 面板扩展：加载 config.json（不存在则作为普通 gost 运行）
	if cfg, err := panel.LoadConfig("config.json"); err == nil {
		logger.Default().Infof("panel config loaded: %s", cfg.Addr)
		panel.SetPanelConfig(cfg)
	} else {
		logger.Default().Warnf("panel config not found, running as standalone gost: %v", err)
	}
